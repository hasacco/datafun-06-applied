...
def inspect_basic(df_countries: pd.DataFrame) -> None:
    ...
    Arguments:
        df_countries: The DataFrame to inspect.
    ...
    column_names: list[str] = list(df_countries.columns)
    ...
    shape: tuple[int, int] = df_countries.shape
...
def build_data_dictionary(df_countries: pd.DataFrame) -> pd.DataFrame:
    ...
    Arguments:
    - df_countries: The DataFrame to analyze.
    ...
    data_dictionary = pd.DataFrame(
        {
            "column": df_countries.columns,
            "dtype": [str(t) for t in df_countries.dtypes],
            "missing_count": df_countries.isna().sum().values,
            "missing_pct": (df_countries.isna().mean() * 100).round(2).values,
        }
    )
...
def check_quality(df_countries: pd.DataFrame) -> None:
    ...
    LOG.debug(f"\n{df_countries.isna().sum().sort_values(ascending=False)}")

    dup_count = int(df_countries.duplicated().sum())
    LOG.info(f"Duplicate rows detected: {dup_count}")

    LOG.info("Call describe() for numeric columns")
    LOG.debug(f"\n{df_countries[SELECTED_NUMERIC_COLS].describe()}\n")


# === Section 5. Create Clean View for EDA ===


def make_clean_view(df_countries: pd.DataFrame) -> pd.DataFrame:
    ...
    Arguments:
    - df_countries: The original DataFrame.
    ...
    df_clean: pd.DataFrame = df_countries.dropna(subset=cols_required).copy()

    # Report what was kept and what was dropped
    count_original: int = df_countries.shape[0]
...
def make_plots(df_clean: pd.DataFrame) -> None:
    ...
    # Create a 2x2 subplot grid for the box plots
    fig, axes = plt.subplots(nrows=2, ncols=2, figsize=(10, 60), sharey=False)
